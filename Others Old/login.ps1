...
# Use Start-Process to avoid HTML entity encoding issues
$tempLoginOutput = Join-Path $env:TEMP "fab_login_$(Get-Random).txt"
$tempLoginError = Join-Path $env:TEMP "fab_login_error_$(Get-Random).txt"

try {
    $loginProcess = Start-Process -FilePath "fab" `
        -ArgumentList "auth", "login", "-u", $clientId, "-p", $clientSecret, "--tenant", $tenantId `
        -NoNewWindow -Wait -PassThru `
        -RedirectStandardOutput $tempLoginOutput `
        -RedirectStandardError $tempLoginError
    ...
    if ($loginExitCode -ne 0) {
        $loginOutput = ""
        $loginError = ""
        if (Test-Path $tempLoginOutput) { $loginOutput = Get-Content $tempLoginOutput -Raw -Encoding UTF8 -ErrorAction SilentlyContinue }
        if (Test-Path $tempLoginError) { $loginError = Get-Content $tempLoginError -Raw -Encoding UTF8 -ErrorAction SilentlyContinue }
        ...
        Write-Host "========================================" -ForegroundColor Red
        Write-Host ""
        if ($loginOutput) { Write-Host "Output:" -ForegroundColor Yellow; Write-Host $loginOutput }
        if ($loginError) { Write-Host "Error:" -ForegroundColor Yellow; Write-Host $loginError }
        Write-Host ""
        Write-Host "Please verify your credentials in the Configuration tab." -ForegroundColor Yellow
        
        Remove-Item $tempLoginOutput -ErrorAction SilentlyContinue
        Remove-Item $tempLoginError -ErrorAction SilentlyContinue
        Read-Host "Press Enter to close this window"
        exit 1
    }
...
# Step 1: List all workspaces using Start-Process
$tempWsOutput = Join-Path $env:TEMP "fab_ws_$(Get-Random).txt"
$tempWsError = Join-Path $env:TEMP "fab_ws_error_$(Get-Random).txt"

try {
    $wsProcess = Start-Process -FilePath "fab" `
        -ArgumentList "ls" `
        -NoNewWindow -Wait -PassThru `
        -RedirectStandardOutput $tempWsOutput `
        -RedirectStandardError $tempWsError
    
    $wsExitCode = $wsProcess.ExitCode
    
    $workspaces = ""
    if (Test-Path $tempWsOutput) {
        $workspaces = Get-Content $tempWsOutput -Raw -Encoding UTF8 -ErrorAction SilentlyContinue
    }
    
    Remove-Item $tempWsOutput -ErrorAction SilentlyContinue
    Remove-Item $tempWsError -ErrorAction SilentlyContinue
    ...
    foreach ($line in $workspaceLines) {
        ...
        if ($line -match '(.+)\.Workspace') {
            ...
            try {
                ...
                # Step 2 & 3: List items in workspace using Start-Process
                Write-Host "Items in workspace:" -ForegroundColor Cyan
                
                $tempItemsOutput = Join-Path $env:TEMP "fab_items_$(Get-Random).txt"
                $tempItemsError = Join-Path $env:TEMP "fab_items_error_$(Get-Random).txt"
                
                try {
                    $itemsProcess = Start-Process -FilePath "fab" `
                        -ArgumentList "ls", "$workspaceName.Workspace" `
                        -NoNewWindow -Wait -PassThru `
                        -RedirectStandardOutput $tempItemsOutput `
                        -RedirectStandardError $tempItemsError
                    
                    $itemsOutputRaw = ""
                    if (Test-Path $tempItemsOutput) {
                        $itemsOutputRaw = Get-Content $tempItemsOutput -Raw -Encoding UTF8 -ErrorAction SilentlyContinue
                    }
                    
                    # Split into lines for processing
                    $itemsOutput = $itemsOutputRaw -split "`r?`n"
                    
                    Remove-Item $tempItemsOutput -ErrorAction SilentlyContinue
                    Remove-Item $tempItemsError -ErrorAction SilentlyContinue
                } catch {
                    Remove-Item $tempItemsOutput -ErrorAction SilentlyContinue
                    Remove-Item $tempItemsError -ErrorAction SilentlyContinue
                    throw
                ...
                if ($itemLine -match '^\s*(.+)\.([\w]+)\s*$' -and $itemLine -notmatch '\.Workspace' -and $itemLine -notmatch '\.Folder' -and $itemLine -notmatch '^\s*\.') {
                    ...
                    try {
                        ...
                        $tempExportOutput = Join-Path $env:TEMP "fab_export_$(Get-Random).txt"
                        $tempExportError = Join-Path $env:TEMP "fab_export_error_$(Get-Random).txt"
                        
                        try {
                            $exportProcess = Start-Process -FilePath "fab" `
                                -ArgumentList "export", "$workspaceName.Workspace/$fullItemName", "-o", $exportPath, "-f" `
                                -NoNewWindow -Wait -PassThru `
                                -RedirectStandardOutput $tempExportOutput `
                                -RedirectStandardError $tempExportError
                            
                            $exportExitCode = $exportProcess.ExitCode
                            
                            $exportOutput = ""
                            $exportError = ""
                            if (Test-Path $tempExportOutput) { $exportOutput = Get-Content $tempExportOutput -Raw -Encoding UTF8 -ErrorAction SilentlyContinue }
                            if (Test-Path $tempExportError) { $exportError = Get-Content $tempExportError -Raw -Encoding UTF8 -ErrorAction SilentlyContinue }
                            
                            Remove-Item $tempExportOutput -ErrorAction SilentlyContinue
                            Remove-Item $tempExportError -ErrorAction SilentlyContinue
                            ...
                            if ($exportExitCode -eq 0) {
                                ...
                                $errorMsg = $null
                                if ($exportError) {
                                    $errorMatch = $exportError | Select-String -Pattern "x export: (.+)" | Select-Object -First 1
                                    if ($errorMatch -and $errorMatch.Matches.Count -gt 0) {
                                ...
                                if ($errorMsg) {
                                    ...
                                    Write-Host "    Export failed (exit code: $exportExitCode)" -ForegroundColor Red
                                    if ($exportOutput) { Write-Host "    Output: $exportOutput" -ForegroundColor DarkRed }
                                    if ($exportError) { Write-Host "    Error: $exportError" -ForegroundColor DarkRed }
                                }
